...
public class DialogSet
{
    ...
    [SerializeField] private List<DialogLine> m_contents = new List<DialogLine>(); 
    ...
    #region Constructor
    public DialogSet(Vector2 _nodePosition, Action<DialogSet> _onRemovePart, GUIStyle _normalStyle, GUIStyle _connectionPointStyle, GUIContent _dialogPartIcon, GUIContent _answerIcon, GUIContent _pointIcon)
    {
        m_partToken = UnityEngine.Random.Range(0, 2147483647); 
        ...
        m_contents = new List<DialogLine>();
    ...
    {
        if (m_contents == null) m_contents = new List<DialogLine>();
        m_contents.Add(new DialogLine());
    ...
    public void Draw(string _lineDescriptor, List<DialogSet> _otherParts, Action<DialogLine> _onOutDialogContentSelected, Action<DialogSet> _onInDialogPartSelected)
    {
        ...
        {
            _c = m_contents[i]; 
            _r.y = _c.Draw(_r.position, _lineDescriptor, RemoveContent, (m_type == DialogPartType.PlayerAnswer || (m_type == DialogPartType.BasicType && i == m_contents.Count - 1)), m_pointIcon, _onOutDialogContentSelected, _otherParts);
        }
    ...
    public void InitEditorSettings(GUIStyle _nodeStyle, GUIStyle _connectionPointStyle, GUIContent _dialogPartIcon, GUIContent _answerIcon, GUIContent _pointIcon ,Action<DialogSet> _onRemovePart)
    {
